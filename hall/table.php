...
<div style="margin-left:15px; width:50%; color:red; font-size:16px; float:left;"><?php echo $nowtime_ ?> <span style="color:blue; float:right;">第<?php echo $week ?>周</span></div>
    <div style="margin-right:15px; text-align:right; width:200px; float:right;">查询第<input type="text" value="<?php
    echo $week;
  ...
  while ($row = mysqli_fetch_array($result))
  {
	  ...
	  $ttime=(int)(($ttime-$begtime-60*60*24*7*($week-1))/(60*60*24))%7; //算出当前是周几呀
  ...
  {?>
    ...
    <th rowspan="3" scope="row"><?php echo $array1[$ii] ?></th>
    <td>上午</td>
    <?php 
		for ($j=0;$j<7;$j++)
		{
